...
const RightSidebar = ({selectedUser}) => {
  ...
  return selectedUser && (
    <div className={`bg-gray-100 text-gray-900 w-full relative overflow-y-scroll 
    ${selectedUser ? 'max-md:hidden' : ''}`}>
      <div className='pt-15 flex flex-col items-center gap-2 text-xs font-light mx-auto'>
        ...
        <p className='px-10 mx-auto text-gray-900enter'>{selectedUser.bio || 'No bio yet'}</p>
      </div>
      ...
      <div className='px-5 text-xs'>
        ...
        {isLoadingImages ? (
          <p className='text-gray-500 text-gray-900enter py-4'>Loading images...</p>
        ) : images.length === 0 ? (
          <p className='text-gray-500 text-gray-900enter py-4'>No images shared yet</p>
        ) : (
      ...
      <button className='absolute bottom-5 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-purple-400 to-violet-600
                text-gray-900 border-none text-sm font-light py-2 px-20 rounded-full cursor-pointer'>
                    Logout
      </button>
