...
const InstitutePanel = () => {
    ...
    return (
        <div className="p-6 bg-gray-100 border border-gray-300 rounded-xl mb-8">
            ...
            {!userData?.instituteName && !userData?.isVerifiedByAdmin ? (
                <form onSubmit={handleRequestInstitute} className="space-y-4">
                    <p className="text-sm text-gray-700">You don't have an institute yet. Request one below.</p>
                    <input
                        type="text"
                        ...
                        required
                        className="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-900 text-sm focus:border-gray-900 outline-none"
                    />
                    <input
                        type="text"
                        ...
                        className="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-900 text-sm focus:border-gray-900 outline-none"
                    />
                    <button type="submit" className="px-4 py-2 bg-gray-900 text-white rounded-lg text-sm hover:bg-black transition-colors">
                        Request Institute
                    </button>
                </form>
            ) : userData?.instituteName && !userData?.isVerifiedByAdmin ? (
                ...
                <div className="space-y-6">
                    <div className="p-4 bg-white border border-green-500/30 rounded-lg">
                        <p className="text-sm text-black-400 font-medium">Active Institute: {userData.instituteName}</p>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="p-4 bg-white rounded-lg border border-gray-300">
                            <form onSubmit={handleCreateClassroom} className="space-y-3">
                                <h4 className="text-sm font-semibold text-gray-900 flex items-center gap-2 mb-3">
                                    <Plus className="h-4 w-4 text-gray-900" /> Create Classroom
                                </h4>
                                <input
                                    type="text"
                                    ...
                                    required
                                    className="w-full px-3 py-2 bg-gray-100 border border-gray-600 rounded-lg text-gray-900 text-sm focus:border-gray-900 outline-none"
                                />
                                <input
                                    type="text"
                                    ...
                                    className="w-full px-3 py-2 bg-gray-100 border border-gray-600 rounded-lg text-gray-900 text-sm focus:border-gray-900 outline-none"
                                />
                                <button type="submit" className="w-full py-2 bg-gray-900 text-white rounded-lg text-sm font-medium hover:opacity-90">
                                    Create Classroom
                                </button>
                            </form>
                        </div>
                        ...
                        <div className="p-4 bg-white rounded-lg border border-gray-300">
                            <form onSubmit={handleAssignUser} className="space-y-3">
                                <h4 className="text-sm font-semibold text-gray-900 flex items-center gap-2 mb-3">
                                    <Users className="h-4 w-4 text-gray-900" /> Assign Teacher/Student to Institute
                                </h4>
                                <p className="text-xs text-gray-500 mb-2">Link an existing user to your institute using their User ID.</p>
                                <input
                                    type="text"
                                    placeholder="User ID"
                                    value={targetUserId}
                        ...
                        <div className="p-4 bg-white rounded-lg border border-gray-300">
                            <form onSubmit={handleAddStudent} className="space-y-3">
                                <h4 className="text-sm font-semibold text-gray-900 flex items-center gap-2 mb-3">
                                    <UserPlus className="h-4 w-4 text-gray-900" /> Add New Student (Admission)
                                </h4>
                                <p className="text-xs text-gray-500 mb-2">Create a new student account. They will receive an email with their login credentials.</p>
                        ...
                        <div className="p-4 bg-white rounded-lg border border-gray-300">
                            <form onSubmit={handleAssignStudentToClassroom} className="space-y-3">
                                <h4 className="text-sm font-semibold text-gray-900 flex items-center gap-2 mb-3">
                                    <BookOpen className="h-4 w-4 text-gray-900" /> Assign Student to Classroom
                                </h4>
                                <p className="text-xs text-gray-500 mb-2">Assign an existing student of your institute to a classroom.</p>
                                <div className="relative">
                                    ...
                                    />
                                    {showStudentDropdown && (
                                        <div className="absolute z-10 w-full mt-1 bg-gray-100 border border-gray-600 rounded-lg max-h-40 overflow-y-auto shadow-xl">
                                            {instituteStudents
                                                .filter(s => s.name.toLowerCase().includes(assignStudentName.toLowerCase()))
                                                .map(student => (
                                                    <div
                                                        key={student._id}
                                                        className="px-3 py-2 text-sm text-white hover:bg-gray-900 cursor-pointer"
                                                        onMouseDown={(e) => {
                                                    ...
                                                    >
                                                        {student.name} <span className="text-gray-500 text-xs ml-1">({student.email})</span>
                                                    </div>
                                                ))}
                                            {instituteStudents.filter(s => s.name.toLowerCase().includes(assignStudentName.toLowerCase())).length === 0 && (
                                                <div className="px-3 py-2 text-sm text-gray-500">No matching students found</div>
                                            )}
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="p-4 bg-white rounded-lg border border-gray-300">
                            <form onSubmit={handleAddTeacher} className="space-y-3">
                                <h4 className="text-sm font-semibold text-gray-900 flex items-center gap-2 mb-3">
                                    <UserPlus className="h-4 w-4 text-gray-900" /> Add New Teacher
                                </h4>
                                <p className="text-xs text-gray-500 mb-2">Create a new teacher account. They will receive an email with their login credentials.</p>
                        ...
                        <div className="p-4 bg-white rounded-lg border border-gray-300">
                            <form onSubmit={handleAssignTeacherToClassroom} className="space-y-3">
                                <h4 className="text-sm font-semibold text-gray-900 flex items-center gap-2 mb-3">
                                    <BookOpen className="h-4 w-4 text-gray-900" /> Assign Teacher to Classroom
                                </h4>
                                <p className="text-xs text-gray-500 mb-2">Assign an existing teacher to a classroom.</p>
                                <div className="relative">
                                    <input
                                        type="text"
                                        ...
                                        <div className="absolute z-10 w-full mt-1 bg-gray-100 border border-gray-600 rounded-lg max-h-40 overflow-y-auto shadow-xl">
                                            {instituteTeachers
                                                ...
                                                .map(teacher => (
                                                    ...
                                                    >
                                                        {teacher.name} <span className="text-gray-500 text-xs ml-1">({teacher.email})</span>
                                                    </div>
                                                ))}
                                            {instituteTeachers.filter(t => t.name.toLowerCase().includes(assignTeacherName.toLowerCase())).length === 0 && (
                                                <div className="px-3 py-2 text-sm text-gray-500">No matching teachers found</div>
                                            )}
                                        </div>
                                    )}
                                </div>
