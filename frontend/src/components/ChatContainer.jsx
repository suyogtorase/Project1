    ...
    if(!selectedUser){
        return (
            <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
                ...
                <p className='text-lg font-medium text-gray-900'>Chat anytime, anywhere</p>
            </div>
        )
    }
        ...
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
          ...
          <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
            ...
            <div className='flex-1 text-gray-900'>
                ...
                <p className='text-xs text-gray-700'>{selectedUser.email}</p>
            </div>
          ...
          <div className='flex flex-col h-[calc(100%-130px)] overflow-y-scroll p-3 pb-28 gap-4'>
            {isLoadingMessages && <p className='text-xs text-gray-500 text-gray-900enter py-4'>Loading messages...</p>}
            {!isLoadingMessages && messages.length === 0 && (
                <p className='text-xs text-gray-500 text-gray-900enter py-4'>Start the conversation</p>
            )}
            {messages.map((msg)=> {
                ...
                return (
                    <div key={messageKey} className={`flex items-end gap-2 ${isMe ? 'flex-row-reverse' : ''}`}>
                        {msg.image ? (
                            <img src={msg.image} alt="" className='max-w-[230px] border border-gray-300 rounded-lg overflow-hidden'/>
                        ) : (
                            <p className={`p-3 max-w-[70%] text-sm font-light rounded-2xl break-words text-gray-900
                                ${isMe ? 'bg-violet-500/60 rounded-br-none' : 'bg-gray-700/70 rounded-bl-none'}`}>
                                {msg.text}
                            </p>
                        )}
                        <div className='text-gray-900enter text-[10px] text-gray-500 flex flex-col items-center gap-1'>
            ...
            <div className='flex items-center gap-3'>
                <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                    <textarea 
                        ...
                        rows={1}
                        className='flex-1 text-sm p-3 border-none rounded-lg outline-none text-gray-900 placeholder-gray-400 bg-transparent resize-none' />
